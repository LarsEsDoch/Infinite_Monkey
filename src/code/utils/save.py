...
def save_text(text):
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(text)
    file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../results/output_absolute.txt"))
    with open(file_path, "a", encoding="utf-8") as f:
        f.write(text)

def save_large_text(chunk):
    os.makedirs("../../results", exist_ok=True)
    file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../results/output.txt"))
...
def save_benchmark(best_length, best_length_search):
